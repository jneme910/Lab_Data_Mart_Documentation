...
go
	  ...
	  SELECT result_key, result_source_key, r.prep_key,  result_type, result_reliability, r.analyte_key, r.procedure_key, r.source_system_key, REPLACE (result_value, '..', '.') AS result_value , p.prep_code  , b.proced_code
FROM dbo.layer AS l
...
LEFT JOIN dbo.bridge_proced_ss AS b ON r.procedure_key = b.procedure_key AND 
            ...
            WHERE (result_type = 'layer result' AND 
		   
		    r.analyte_key IN (568,652,653,721,722,723,791,1291,1292,1440,2099,1154,1289,1442, 1290, 1152,1288,1441, 1160, 1161,1287, 518,654,655,736,737,738,796,1303,1304,1450,2101, 1157,1301,1452, 1302, 1046,1300,1451, 1159, 517,619,650,706,707,708,748,749,750,751,752,753,799,1097,1313,1314,1315,1316,1453,1454,2026,2030,2031,2032,2102, 1158,1309,1310,1455,1456,2034,2035,1311,1312,2029,1048,1307,1308,1457,1458,2028,2033,11671168,1305,1306,2027, 512,618,648,703,704,705,715,716,717,718,719,720,790,1096,1325,1326,1327,1328,1462,1463,2036,2041,2042,2043,2104,1153,1321,1322,1466,1467,2039,2045,1323,1324,2040,1043,1319,1320,1464,1465,2038,2044, 1169, 1170,1317,1318,2037, 514,658,659,660,661,662,663,664,665,724,728,730,731,732,792,1459,2103, 727,1155,1461, 729, 1163, 1164,725, 513,666,667,733,734,735,793,1297,1298,1447,2100, 1156,1295,1449, 1296, 1045,1294,1448, 1165, 1166,1293, 794,1070,1238, 2021, 2022, 559, 647, 1078, 1076, 2086, 1077, 1079, 1071, 1090, 1851, 1072, 1091, 1848, 1095, 1849, 1074, 1093, 1850, 1075, 1094, 1846, 1073, 1092, 1847, 623, 624, 1930, 570,1030,1522,2096, 1171,1329,1523,2097, 1172, 1173, 531,620,651,668,669,670,675,676,677,754,755,756,757,758,759,760,761,762,800,801,1444,1445,1446,1512,1513,1514,1515,1516,1517,1518, 1174,1177, 1183, 1175,1178, 1176,1179, 563,636,803,1520,2094, 1180,1521,2095, 1181, 1182, 1887, 1888
...
FROM result_light AS result WHERE (prep_code IS NOT NULL  AND (result.result_source_key = q1.result_source_key) )) 
...
--Sum_of_nh4_ph_7_Ext_bases (Calc)							
							...
							(SELECT        TOP (1) CAST(ROUND (  CAST (result_value AS REAL), 1) AS decimal(10, 1)) AS Expr1
...
--aluminum_saturation (calc)						
														...
														(SELECT        TOP (1) CAST(ROUND (  CAST (result_value AS REAL), 0) AS decimal(10, 0)) AS Expr1
...
(SELECT        TOP (1) CAST(ROUND (  CAST (result_value AS REAL), 0) AS decimal(10, 0)) AS Expr1
							...
							(SELECT        TOP (1) CAST(ROUND (  CAST (result_value AS REAL), 2) AS decimal(10, 2)) AS Expr1
...
SELECT DISTINCT 
labsampnum,result_source_key,prep_code,sum_of_nh4_ph_7_Ext_bases,sum_of_cations_cec_pH_8_2,ecec_base_plus_aluminum,aluminum_saturation,base_sat_sum_of_cations_ph_8_2,base_sat_nh4oac_ph_7, estimated_organic_carbon, carbon_to_nitrogen_ratio, aluminum_plus_half_iron_oxalat,caco3_lt_20_mm,gypsum_lt_20_mm,ca_to_mg_ratio,total_estimated_salts_satx,exchangeable_sodium,sodium_absorption_ratio,phosphorus_anion_resin_capacit
INTO lab_chemical_properties_layer_results_1
FROM #tempXX
GROUP BY labsampnum,result_source_key,prep_code,sum_of_nh4_ph_7_Ext_bases,sum_of_cations_cec_pH_8_2,ecec_base_plus_aluminum,aluminum_saturation,base_sat_sum_of_cations_ph_8_2,base_sat_nh4oac_ph_7, estimated_organic_carbon, carbon_to_nitrogen_ratio, aluminum_plus_half_iron_oxalat,caco3_lt_20_mm,gypsum_lt_20_mm,ca_to_mg_ratio,total_estimated_salts_satx,exchangeable_sodium,sodium_absorption_ratio,phosphorus_anion_resin_capacit

CREATE INDEX  lab_chemical_properties_layer_results_1_idx1c ON  lab_chemical_properties_layer_results_1 (labsampnum, result_source_key) INCLUDE (prep_code)
CREATE INDEX  lab_chemical_properties_layer_results_1_result_source_key_idx2 ON  lab_chemical_properties_layer_results_1 (result_source_key )
CREATE INDEX lab_chemical_properties_layer_results_1_prep_code_idx5 ON  lab_chemical_properties_layer_results_1 (prep_code)
